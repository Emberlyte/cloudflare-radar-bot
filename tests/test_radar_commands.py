...
async def test_show_attacks_layer3_success(mock_callback, mock_radar_client):
...
async def test_show_attacks_layer7_success(mock_callback, mock_radar_client):
    mock_radar_client.attacks_layer7_summary.return_value = {
        "summary_0": {"GET": "81.1", "POST": "15.1"}
    }

    await show_attacks_layer7(mock_callback, mock_radar_client)
...
async def test_show_dns_success(mock_callback, mock_radar_client):
    mock_radar_client.dns_by_protocol_summary.return_value = {
        "summary_0": {"UDP": "84.6", "TLS": "7.0", "HTTPS": "6.2", "TCP": "2.2"}
    }

    await show_dns(mock_callback, mock_radar_client)
...
async def test_show_email_threats_success(mock_callback, mock_radar_client):
    mock_radar_client.email_threat_category_summary.return_value = {
        "summary_0": {"Link": "67.5", "Scam": "65.4"}
    }

    await show_email_threats(mock_callback, mock_radar_client)
...
async def test_show_attacks_layer3_rate_limited(mock_callback, mock_radar_client):
